...
**Tomas's rule for the ground, added the same evening and moved once: from resolution 9 inward,
the node's own plan.** He asked whether the generated drawing from the map fetch could be used from
stop 8; it was tried at 8, he looked, and said 9 works better. It does, and the geometry says why:
the plan is about 3 km across, the resolution-8 plate is 4.2 km, so at 8 the plan sits in the middle
with paper round it, while the resolution-9 plate is 1.6 km and the plan fills it edge to edge. So
from 9 the plan — every vertex the node's own, nothing sent; at 8 and coarser the tiles, until the
frame is wider than anything the node has mapped and the plan is a rectangle. A pressed base always
wins over the rule; pressing the base the rule would have chosen clears the press, so a stale choice
does not outlive the dial. The wall follows the same rule stop by stop. Measured: at 8, where the
dial opens, the ground row reads **12 requests** and the fold is 48.7 % empty at 1440; one stop in,
at 9, it reads **0 requests** and 63.3 % — a drawing of buildings is emptier than a photograph of
