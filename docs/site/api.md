...
- `digest` is keyed by stage, `observe`, `decide`, `act` and `measure`, and each stage holds one sentence per locale. `digest.prompts` is three short questions per locale for the ask pane's composer, written from the bundle. `digest.simple` is one paragraph per locale, three sentences: the house's own stations and the nearest others, the oldest open alert, and how many times the node asked in the last 30 days and how many were answered. The dashboard's simple mode draws `digest.simple`.
...
`{model, rung, where, host, running, why, rungs, leaves, tools, stored}`: the model the pane asks first and its `rung` (`local`, `remote` or `online`, from Set up → agent and `AGENT_PREFER`), `where` it runs (`this machine`, `your network` or `online`) and its `host`, whether it is answering, why not when it is not, every model in the order it is tried as `{rung, model, host, where}`, `leaves` (true when an online model is in that order, so the question and the page's context may leave the network), the tools it is offered as `{name, class}`, and `stored`, which is always `false`. 404 when no model is set up at all: no loop on this machine (the `agent` profile, which `planetai agent local` adds), no remote model, and no online one that `AGENT_PREFER` allows; its `detail` carries `recommend` (`{tag, size, memory_gb, pull}`, the tag `planetai agent local` would suggest for this machine's memory), `mcp` and `token_hint`, for the page's no-model state. With Ollama installed and down, `running` is `false` and the tag is still named.
