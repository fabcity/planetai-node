...
v0.72.1; each would be a code pack or an adapter behind a flag, off by default. Nothing that needs an account runs
...
model sensors carry their attribution or licence in `meta` on `/sensors`; the daily export states only its own
CC BY 4.0.
