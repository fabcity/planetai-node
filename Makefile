...
backup:  ; ./backup.sh
lint:    ; bash -n install install.sh backup.sh update.sh bin/planetai && python3 tools/check_sql.py && python3 -m py_compile app/*.py && python3 -c "import yaml;yaml.safe_load(open('config/rules.yml'));yaml.safe_load(open('docker-compose.yml'))" && echo ok
