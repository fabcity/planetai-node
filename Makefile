...
lint:    ; bash -n install install.sh backup.sh update.sh bin/planetai && python3 tools/check_sql.py && python3 tools/check_cli_python.py && python3 tools/check_docs.py && python3 tools/check_ui.py && bash tools/check_requirements.sh && (python3 -c 'import sqlglot' 2>/dev/null && python3 tools/check_rules.py || echo '  - rule check skipped (pip install sqlglot)') && python3 -m py_compile app/*.py && (python3 -c 'import pyflakes' 2>/dev/null && python3 -m pyflakes app/*.py packs/*/*.py | grep -v 'imported but unused' | grep . && exit 1 || true) && python3 -c "import yaml;yaml.safe_load(open('config/rules.yml'));yaml.safe_load(open('docker-compose.yml'))" && (if python3 -c 'import fastapi' 2>/dev/null; then $(MAKE) -s import-check; else echo '  - import check skipped (pip install -r app/requirements.txt to enable)'; fi) && echo ok
test:    ; mkdir -p /tmp/stub && echo "class Client: pass" > /tmp/stub/httpx.py && PYTHONPATH=/tmp/stub:app python3 tests/test_sources.py && PYTHONPATH=/tmp/stub:app python3 tests/test_nearby.py && PYTHONPATH=/tmp/stub:app python3 tests/test_forecast.py && PYTHONPATH=/tmp/stub:app python3 tests/test_meshtastic.py && PYTHONPATH=/tmp/stub:app python3 tests/test_logic.py && PYTHONPATH=/tmp/stub:app python3 tests/test_packs.py && PYTHONPATH=/tmp/stub:app python3 tests/test_settings.py && PYTHONPATH=/tmp/stub:app python3 tests/test_report_schedule.py && PYTHONPATH=/tmp/stub:app python3 tests/test_report_templates.py && python3 tests/test_shipped.py && python3 tests/test_check_ui.py && python3 tests/test_ground.py && PYTHONPATH=/tmp/stub:app python3 tests/test_briefing.py && PYTHONPATH=/tmp/stub:app python3 tests/test_earth.py
