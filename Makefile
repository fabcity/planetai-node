...
test:    ; bash tests/test_release_consistency.sh && bash tests/test_ship_gate.sh && bash tests/test_daemon_state.sh && bash tests/test_diagnose.sh && bash tests/test_remove.sh && bash tests/test_sudo_prompt.sh && bash tests/test_running_state.sh && bash tests/test_preflight.sh && bash tests/test_geocode.sh && mkdir -p /tmp/stub && echo "class Client: pass" > /tmp/stub/httpx.py && PYTHONPATH=/tmp/stub:app python3 tests/test_sources.py && PYTHONPATH=/tmp/stub:app python3 tests/test_nearby.py && PYTHONPATH=/tmp/stub:app python3 tests/test_forecast.py && PYTHONPATH=/tmp/stub:app python3 tests/test_meshtastic.py && PYTHONPATH=/tmp/stub:app python3 tests/test_logic.py && PYTHONPATH=/tmp/stub:app python3 tests/test_packs.py && PYTHONPATH=/tmp/stub:app python3 tests/test_settings.py && PYTHONPATH=app python3 tests/test_ladder.py && PYTHONPATH=/tmp/stub:app python3 tests/test_report_schedule.py && PYTHONPATH=/tmp/stub:app python3 tests/test_report_templates.py && python3 tests/test_shipped.py && PYTHONPATH=app python3 tests/test_share.py && python3 tests/test_check_ui.py && python3 tests/test_dashboard.py && python3 tests/test_ground.py && PYTHONPATH=/tmp/stub:app python3 tests/test_briefing.py && PYTHONPATH=/tmp/stub:app python3 tests/test_earth.py
