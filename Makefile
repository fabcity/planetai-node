...
.PHONY: up update bootstrap down restart logs health stats alerts cells rho backup lint test ship released check-floors docs learn
up:      ; ./install.sh
update:  ; ./update.sh
...
lint:    ; bash -n install install.sh backup.sh update.sh bin/planetai tools/preflight.sh tools/ship.sh tools/session.sh && python3 tools/check_floors.py && python3 tools/gen_floors.py --check && python3 tools/gen_defaults.py --check && python3 tools/render_platforms.py --check && python3 tools/extract_strings.py --check && python3 tools/check_sql.py && python3 tools/check_cli_python.py && python3 tools/check_docs.py && python3 tools/check_registry.py && python3 tools/check_ui.py && python3 tools/build_learn.py --check && python3 tools/build_docs.py --check && python3 tools/check_site.py && python3 tools/check_wire.py && python3 tools/check_theme.py && bash tools/check_requirements.sh && (if python3 -c 'import sqlglot' 2>/dev/null; then python3 tools/check_rules.py; else echo '  - rule check skipped (pip install sqlglot)'; fi) && python3 -m py_compile app/*.py app/issues/*.py && (python3 -c 'import pyflakes' 2>/dev/null && python3 -m pyflakes app/*.py app/issues/*.py packs/*/*.py | grep -v 'imported but unused' | grep . && exit 1 || true) && python3 -c "import yaml;yaml.safe_load(open('config/rules.yml'));c=yaml.safe_load(open('docker-compose.yml'));[yaml.safe_load(open(w)) for w in __import__('glob').glob('.github/workflows/*.yml')];b=[n for n,v in c['services'].items() if 'env_file' in v and n not in ('app','agent')];assert not b, 'env_file: .env hands a container every secret the node holds. Only app and agent may: '+', '.join(b)+'. List the keys it reads under environment: instead';print('  only app and agent are handed the whole .env')" && python3 -c "import re,glob;e=open('.env.example').read();assert not re.search(r'^[A-Z_]+=[ \t]+#', e, re.M), 'a comment after a key that ships blank becomes its value: move it above the key';u=[f for f in sorted(glob.glob('packs/*/*.py')) if 'httpx.Client()' in open(f).read()];assert not u, 'httpx.Client() with no timeout, and httpx has no read timeout by default: '+', '.join(u);print('  .env.example comments are above their keys; every pack http client has a timeout')" && (if python3 -c 'import fastapi' 2>/dev/null; then $(MAKE) -s import-check; else echo '  - import check skipped (pip install -r app/requirements.txt to enable)'; fi) && echo ok
